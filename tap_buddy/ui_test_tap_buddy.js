context("TAP Buddy", () => {
	...
	it("creates campaign and auto-fills message template", () => {
		...
		// Some datepickers don't accept typing reliably in CI; ensure the value on the client form.
		cy.window().then((win) => {
			if (win.cur_frm && typeof win.cur_frm.set_value === "function") {
				win.cur_frm.set_value("send_date", "2000-01-01 00:00:00");
			}
		});
		...
		cy.wait("@save_call");
		cy.intercept("POST", "/api/method/frappe.desk.form.save.savedocs").as("submit_call");
