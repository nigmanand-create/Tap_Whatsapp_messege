CREATE OR REPLACE TABLE FUNCTION `central-phalanx-297915.MEL_datasets.get_feedback_context_v1`(json_payload STRING)
AS (
  WITH params AS (
    SELECT
  ...
  monthly_stats AS (
    SELECT
      ...
      e.v_lang,
      SAFE_DIVIDE(
        COUNTIF(s.portal_accessed_this_month),
        NULLIF(COUNT(DISTINCT s.student_id), 0)
      ) * 100.0                                   AS access_rate,
      SAFE_DIVIDE(
        COUNTIF(s.assignment_submitted_this_month),
        NULLIF(COUNT(DISTINCT s.student_id), 0)
      ) * 100.0                                   AS submission_rate,
      MAX(s.tracking_week)                         AS week_number,
      AVG(SAFE_DIVIDE(
        COUNTIF(s.portal_accessed_this_month) OVER (),
        NULLIF(COUNT(DISTINCT s.student_id) OVER (), 0)
      ) * 100.0)                                  AS state_access_rate
    FROM educator e
  ...
  SELECT
    ...
    -- school_metrics: localized narrative badge for this school's performance
    CASE v_lang
      WHEN 'hi' THEN
        CASE
    ...
    -- program_metrics: localized state-average comparison narrative
    CASE v_lang
      WHEN 'hi' THEN
        CASE
          WHEN IFNULL(ROUND(access_rate, 1), 0.0) >= IFNULL(ROUND(state_access_rate, 1), 0.0) THEN 'राज्य औसत से ऊपर'
          ELSE 'राज्य औसत से नीचे'
        END
      WHEN 'mr' THEN
        CASE
          WHEN IFNULL(ROUND(access_rate, 1), 0.0) >= IFNULL(ROUND(state_access_rate, 1), 0.0) THEN 'राज्य सरासरीपेक्षा वर'
          ELSE 'राज्य सरासरीपेक्षा कमी'
        END
      ELSE
        CASE
          WHEN IFNULL(ROUND(access_rate, 1), 0.0) >= IFNULL(ROUND(state_access_rate, 1), 0.0) THEN 'Above State Average'
          ELSE 'Below State Average'
        END
    END AS program_metrics
  FROM monthly_stats
  LIMIT 1
);
